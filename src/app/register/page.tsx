...
import { ChevronLeft, ChevronRight, CheckCircle2, Loader2, Landmark } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({
    full_name: "", rif: "", n_accionistas: "", address: "",
    municipality: "", home_number: "", phone_number: "", email: "",
    social_object: "", products: "", start_date: "", experience: "",
    comercial_name: "", workers: "", workers_to_date: "", facturation_last_year: "",
    mercantil: "", capital_suscribed: "", capital_paid: "", dni: "", company_position: "",
    n_actions: "", actions_type: "", amount: "", password: "", password_confirm: "",
  });

  const inputClass = "w-full bg-slate-100 border border-slate-300 text-slate-700 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 transition text-sm";
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!form.n_accionistas.trim()) newErrors.n_accionistas = "Campo requerido";
    ...
    if (step === 2) {
      if (!form.social_object.trim())        newErrors.social_object        = "Campo requerido";
      ...
      if (!form.comercial_name.trim())       newErrors.comercial_name       = "Campo requerido";
      if (!form.workers.trim())              newErrors.workers              = "Campo requerido";
      if (!form.workers_to_date.trim())      newErrors.workers_to_date      = "Campo requerido";
      if (!form.facturation_last_year.trim()) newErrors.facturation_last_year = "Campo requerido";
    }
    if (step === 3) {
      if (!form.mercantil.trim())         newErrors.mercantil         = "Campo requerido";
      if (!form.capital_suscribed.trim()) newErrors.capital_suscribed = "Campo requerido";
      if (!form.capital_paid.trim())      newErrors.capital_paid      = "Campo requerido";
      if (!form.dni.trim())               newErrors.dni               = "Campo requerido";
      if (!form.company_position.trim())  newErrors.company_position  = "Campo requerido";
    }
    if (step === 4) {
      ...
      if (!form.password_confirm.trim()) newErrors.password_confirm = "Campo requerido";
  ...
  const nextStep = () => { if (validateStep() && step < totalSteps) setStep(step + 1); };
  const prevStep = () => { if (step > 1) setStep(step - 1); };

  const emptyForm = {
    full_name: "", rif: "", n_accionistas: "", address: "", municipality: "", home_number: "", phone_number: "", email: "",
    social_object: "", products: "", start_date: "", experience: "", comercial_name: "", workers: "", workers_to_date: "", facturation_last_year: "",
    mercantil: "", capital_suscribed: "", capital_paid: "", dni: "", company_position: "",
    n_actions: "", actions_type: "", amount: "", password: "", password_confirm: "",
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      setServerError("");
      await axios.post("https://fedeinversiones-mvp.onrender.com/api/auth/register/", form, { timeout: 30000 });
      alert("Empresa registrada correctamente");
      setForm(emptyForm);
      setStep(1);
    } catch (error: any) {
      if (error.response?.data?.errors) setErrors(error.response.data.errors);
      setServerError(error.response?.data?.message || "Error al registrar empresa");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950 flex flex-col">
      ...
      <header className="border-b border-white/10 px-4 sm:px-8 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-white font-bold text-base leading-none">FEDEINVERSIONES</h1>
      ...
      <div className="flex-1 flex items-start justify-center px-4 py-8 sm:py-10">
        <div className="w-full max-w-3xl bg-white border border-slate-200 rounded-3xl shadow-2xl p-6 sm:p-8">
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-700">Crear Cuenta</h2>
            <p className="text-emerald-600 text-sm font-medium mt-1">{stepTitles[step - 1]}</p>
          </div>

          {/* Step bar */}
          <div className="grid grid-cols-4 gap-2 sm:gap-3 mb-6 sm:mb-8">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className={`h-2 rounded-full transition-all ${step >= item ? "bg-emerald-500" : "bg-slate-200"}`} />
            ))}
          </div>

          <p className="text-xs sm:text-sm text-slate-500 mb-5">Paso {step} de {totalSteps}</p>

          {serverError && (
            <div className="mb-5 bg-red-100 text-red-700 p-3 rounded-xl text-sm">{serverError}</div>
          )}
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              {[
                { label: "Nombres y Apellidos / Razón Social", name: "full_name", type: "text" },
                { label: "RIF", name: "rif", type: "text" },
                { label: "N° Accionistas", name: "n_accionistas", type: "number" },
                ...
                <div key={name}>
                  ...
                  <input type={type} name={name} value={(form as any)[name]} onChange={handleChange} className={inputClass} />
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              {[
                ...
                { label: "Fecha Constitución", name: "start_date", type: "date" },
                { label: "Experiencia Sector", name: "experience", type: "text" },
                { label: "Nombre Comercial", name: "comercial_name", type: "text" },
                { label: "N° Trabajadores", name: "workers", type: "number" },
                { label: "A la Fecha", name: "workers_to_date", type: "date" },
                { label: "Facturación Último Año (Bs.)", name: "facturation_last_year", type: "text" },
              ].map(({ label, name, type }) => (
                <div key={name}>
                  <label className={labelClass}>{label}</label>
                  <input type={type} name={name} value={(form as any)[name]} onChange={handleChange} className={inputClass} />
          ...
          {step === 3 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              {[
                { label: "Datos Registro Mercantil", name: "mercantil", type: "text" },
                { label: "Capital Social Suscrito", name: "capital_suscribed", type: "number" },
                { label: "Capital Social Pagado", name: "capital_paid", type: "number" },
                { label: "Cédula Representante Legal", name: "dni", type: "number" },
                { label: "Cargo en la Empresa", name: "company_position", type: "text" },
              ].map(({ label, name, type }) => (
                <div key={name}>
                  <label className={labelClass}>{label}</label>
                  <input type={type} name={name} value={(form as any)[name]} onChange={handleChange} className={inputClass} />
          ...
          {step === 4 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
              <div>
                ...
                <input name="n_actions" value={form.n_actions} onChange={handleChange} className={inputClass} />
                <ErrorText field="n_actions" />
              </div>
              <div>
                <label className={labelClass}>Tipo Acciones</label>
                <input name="actions_type" value={form.actions_type} onChange={handleChange} className={inputClass} />
              ...
              <div className="sm:col-span-2">
                ...
                <input type="password" name="password_confirm" value={form.password_confirm} onChange={handleChange} className={inputClass} />
              ...
              <div className="sm:col-span-2 bg-emerald-50 border border-emerald-200 rounded-2xl p-4 sm:p-5 text-emerald-700 flex gap-3 items-start text-sm">
                ...
                <p>Al finalizar, los datos serán enviados para revisión administrativa.</p>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-8 sm:mt-10 gap-3">
            ...
            {step < totalSteps ? (
              ...
              >
                {loading ? <><Loader2 className="animate-spin" size={18} />Enviando...</> : "Registrar Empresa"}
              </button>
            )}
          </div>
        </div>
