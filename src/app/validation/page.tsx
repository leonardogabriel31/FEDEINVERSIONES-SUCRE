...
import {
  ...
  X, CheckCircle2, Eye, Users, Mail, BadgeCheck, CalendarDays, UserCircle, Menu,
} from "lucide-react";
...
export default function ValidacionPage() {
  ...
  const handleValidar = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || err.response?.data?.detail || "Error al validar el usuario.");
    } finally {
      setValidating(false);
    }
  };
              ...
              <thead>
                <tr className="border-b border-white/10">
                  {["Nombre completo", "Correo", "Rol", "Registro", "Acciones"].map((h) => (
                    <th key={h} className="px-4 sm:px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">
                      {h}
                    </th>
                  ))}
                </tr>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 sm:py-5 border-b border-white/10">
              ...
              {!validating && (
                <button onClick={() => { setSelected(null); setConfirmed(false); }} className="text-slate-500 hover:text-white transition">
                  <X size={20} />
                </button>
              )}
            </div>
            ...
            <div className="px-5 sm:px-6 py-5 sm:py-6 space-y-4">
              <div className="flex items-center gap-4 pb-4 border-b border-white/10">
                ...
                <div>
                  <p className="text-white font-bold text-base sm:text-lg leading-tight">{selected.full_name}</p>
              ...
              {[
                ...
                <div key={label} className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 flex-shrink-0">{icon}</div>
                  <div>
